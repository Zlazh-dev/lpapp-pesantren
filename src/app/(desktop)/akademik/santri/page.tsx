...
export default function AkademikSantriPage() {
    ...
    const handleAddByNis = () => {
        ...
        setAddError('')
        setAddSuccess('')
        addMut.mutate({ nis: nisInput.trim(), section: 'AKADEMIK' })
    }

    const { data, isLoading, error } = listQuery
    const activeFilterCount = (filterClass ? 1 : 0) + (filterGender ? 1 : 0)
    ...
    const filteredData = filterGender
        ? (data?.items ?? []).filter((s: any) => s.gender === filterGender)
        : (data?.items ?? [])
        ...
        <div className="bg-white rounded-lg border border-gray-200">
            ...
            <div className="p-4 border-b border-gray-200">
                <div className="flex items-center justify-between">
                    <div>
                        <h2 className="text-sm font-bold text-gray-900">Data Santri — Madrasah</h2>
                        ...
                        <div className="relative" ref={kebabRef}>
                            ...
                            {showKebab && (
                                <div className="absolute right-0 top-full mt-1 z-50 w-48 rounded-lg border border-gray-200 bg-white shadow-lg overflow-hidden animate-fade-in">
                                    <button onClick={() => {
                                        ...
                                        const rows = items.map((s: any) => {
                                            ...
                                            return {
                                                ...
                                                'Kode Pos': addr.kodepos || '',
                                                'Kamar': s.dormRoom?.name || '', 'Kelas': s.classGroup?.name || '',
                                            }
                                        })
                                        const ws = XLSX.utils.json_to_sheet(rows)
                                        const wb = XLSX.utils.book_new()
                                        XLSX.utils.book_append_sheet(wb, ws, 'Data Santri Madrasah')
                                        XLSX.writeFile(wb, 'data_santri_madrasah.xlsx')
                                        setShowKebab(false)
                                    ...
                                    <button onClick={() => { router.push('/akademik/santri/scanner'); setShowKebab(false) }} className="w-full flex items-center gap-3 px-3 py-2.5 text-xs text-gray-700 hover:bg-gray-50 transition border-t border-gray-100">
                                        <svg className="w-3.5 h-3.5 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 4v1m6 11h2m-6 0h-2v4m0-11v3m0 0h.01M12 12h4.01M16 20h4M4 12h4m12 0h.01M5 8h2a1 1 0 001-1V5a1 1 0 00-1-1H5a1 1 0 00-1 1v2a1 1 0 001 1zm12 0h2a1 1 0 001-1V5a1 1 0 00-1-1h-2a1 1 0 00-1 1v2a1 1 0 001 1zM5 20h2a1 1 0 001-1v-2a1 1 0 00-1-1H5a1 1 0 00-1 1v2a1 1 0 001 1z" /></svg>
                                        Scan QR Santri
                                    </button>
                                    <button onClick={() => { router.push('/akademik/santri/upload'); setShowKebab(false) }} className="w-full flex items-center gap-3 px-3 py-2.5 text-xs text-gray-700 hover:bg-gray-50 transition border-t border-gray-100">
                ...
                <div className="flex items-center gap-2">
                    <div className="flex-1 relative">
                        ...
                        <input
                            ...
                            className="w-full pl-9 pr-3 py-1.5 bg-white border border-gray-200 rounded text-xs text-gray-900 placeholder-gray-400 focus:border-blue-500 focus:ring-1 focus:ring-blue-500 outline-none"
                        />
                    </div>
                    <div className="relative" ref={filterRef}>
                        ...
                        >
                            ...
                            Filter
                            {activeFilterCount > 0 && (
                                <span className="min-w-[16px] h-4 rounded-full bg-blue-600 text-white text-[9px] font-bold flex items-center justify-center px-1">{activeFilterCount}</span>
                            )}
                        </button>
                        ...
                        {showFilterPanel && (
                            <div className="absolute right-0 top-full mt-1 z-50 w-64 rounded-lg border border-gray-200 bg-white shadow-xl overflow-hidden animate-fade-in">
                                <div className="px-3 py-2.5 border-b border-gray-100 flex items-center justify-between">
                                    ...
                                    {activeFilterCount > 0 && (
                                        <button onClick={() => { setFilterClass(''); setFilterGender(''); setPage(1) }}
                        ...
                        {error ? (
                            ...
                            filteredData.map((santri: any, idx: number) => {
                                ...
                                return (
                                    <tr
                                        key={santri.id}
                                        onClick={() => router.push(`/akademik/santri/${santri.id}`)}
                                    ...
                                    >
                                        ...
                                        <td className="px-4 py-2.5">
                                            <p className="text-xs font-semibold text-gray-900 group-hover:text-blue-700 transition truncate max-w-[200px]">{santri.fullName}</p>
                                        ...
                                        <td className="px-4 py-2.5">
                                            {jenjang ? (
                                                <span className="inline-block px-2 py-0.5 bg-blue-100 text-blue-700 rounded text-[10px] font-semibold">{jenjang}</span>
                                        ...
                                        <td className="px-4 py-2.5">
                                            <span className={`inline-flex items-center justify-center w-5 h-5 rounded-full text-[10px] font-bold ${santri.gender === 'L' ? 'bg-blue-100 text-blue-700' : 'bg-pink-100 text-pink-700'}`}>
                                                {santri.gender}
                                            </span>
                                        </td>
                                        <td className="px-4 py-2.5" onClick={e => e.stopPropagation()}>
                                            <div className="flex items-center justify-center">
                                                <button
                                                    onClick={() => router.push(`/akademik/santri/${santri.id}`)}
                                                    className="w-7 h-7 bg-gray-100 hover:bg-blue-100 hover:text-blue-700 text-gray-500 rounded flex items-center justify-center transition"
                                                    title="Lihat Detail"
                                                >
            ...
            {showAddModal && typeof document !== 'undefined' && createPortal(
                <div className="fixed inset-0 z-[9999] bg-black/40 backdrop-blur-sm flex items-center justify-center p-4" onClick={() => setShowAddModal(false)}>
                    <div className="bg-white rounded-2xl border border-slate-200 shadow-2xl w-full max-w-md p-6 space-y-4" onClick={(e) => e.stopPropagation()}>
                        <div className="flex items-center justify-between">
                            <div>
                                <h2 className="text-base font-bold text-gray-900">Tambah Santri ke Madrasah</h2>
                        ...
                        <div className="flex gap-2">
                            <input value={nisInput} onChange={(e) => setNisInput(e.target.value)}
                                ...
                                autoFocus
                                className="flex-1 rounded-lg border border-gray-200 bg-gray-50 px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500/30 focus:border-blue-500 transition" />
