...
export default function AkademikSantriRequestPage({ params }: { params: Promise<{ id: string }> }) {
    const { id } = use(params)
    ...
    return (
        <div className="max-w-2xl mx-auto space-y-6 animate-fade-in">
            ...
            <div className="flex items-center gap-3">
                <Link href={`/akademik/santri/${id}`} className="p-2.5 rounded-xl border border-slate-200 bg-white text-slate-400 hover:text-blue-600 hover:border-blue-200 hover:shadow-sm transition-all">
            ...
            {submitted ? (
                <div className="rounded-2xl border border-emerald-200 bg-emerald-50 p-8 text-center">
                    ...
                    <div className="flex justify-center gap-3 mt-5">
                        ...
                        <Link href={`/akademik/santri/${id}`} className="px-4 py-2 rounded-xl bg-blue-600 text-white text-sm font-medium hover:bg-blue-700 transition-all">
                            ← Kembali ke Detail
                        </Link>
                    </div>
                </div>
                ...
                <form onSubmit={handleSubmit} className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm space-y-5">
                    ...
                    <div>
                        ...
                        <textarea value={description} onChange={(e) => setDescription(e.target.value)}
                            ...
                            rows={5}
                            className="w-full rounded-xl border border-slate-200 bg-slate-50/50 px-4 py-3 text-sm text-slate-800 placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-blue-500/30 focus:border-blue-500 transition-all resize-none" />
                    ...
                    <button type="submit" disabled={description.length < 10 || createMut.isPending}
                        className="w-full py-3 rounded-xl bg-gradient-to-r from-blue-500 to-indigo-500 text-white text-sm font-semibold hover:opacity-90 transition-all shadow-lg shadow-blue-500/25 disabled:opacity-50 disabled:cursor-not-allowed">
