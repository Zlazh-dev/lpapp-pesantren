...
export default async function PaymentProofByPaymentPage({
    params,
}: {
    ...
    if (!normalizedId) {
        return (
            <div className="mx-auto max-w-3xl rounded-2xl border border-red-200 bg-white p-6">
                <h1 className="text-lg font-semibold text-slate-900">ID Pembayaran Tidak Valid</h1>
                <p className="mt-2 text-sm text-slate-600">
                    ID pembayaran tidak ditemukan pada URL.
                </p>
                <div className="mt-4">
                    <Link
                        href="/billing/rekap"
    ...
    const proofKind = detectPaymentProofKind(payment.proofUrl)
    ...
    return (
        <div className="mx-auto w-full max-w-5xl space-y-4">
            ...
            <div className="rounded-2xl border border-slate-200 bg-white p-4">
                <dl className="grid grid-cols-1 gap-3 text-sm text-slate-600 md:grid-cols-4">
                    <div>
                        <dt className="text-xs uppercase tracking-wide text-slate-500">Santri</dt>
                        <dd className="mt-1 font-medium text-slate-800">
                            {payment.invoice?.santri?.fullName ?? '-'}
                            {payment.invoice?.santri?.nis ? (
                                <span className="ml-1 font-mono text-xs text-slate-500">({payment.invoice.santri.nis})</span>
                            ) : null}
                        </dd>
                    </div>
                    <div>
                        ...
                        <dd className="mt-1 font-medium text-slate-800">
                            {payment.invoice?.billingModel?.name ?? '-'}
                        </dd>
                    </div>
                    <div>
                    ...
                    <div>
                        <dt className="text-xs uppercase tracking-wide text-slate-500">Metode</dt>
                        <dd className="mt-1 font-medium text-slate-800">{payment.method}</dd>
                    </div>
                </dl>
            </div>
            ...
            <PaymentProofViewer
                ...
                proofKind={proofKind}
                fileUrl={`/api/billing/payments/${payment.id}/proof-file`}
            />
        </div>
    )
}
